import files_function_library
import log_record_functions
...
if folder_selected:
    ...
    log_record_functions.write_log_file("User select destiny folder " + user_dest_path)
    folder_dest_selected = True
else:
    text = "User cancelled Files Destination Folder selection."
    endtext = text
    log_record_functions.write_log_file(text)
...
if folder_selected:
    ...
    log_record_functions.write_log_file("User select originals folder " + user_orig_path)
...
if response:
    log_record_functions.write_log_file("User confirmed erase originals")
    pref_erase_origin = True
else:
    log_record_functions.write_log_file("User cancel erase originals")


#only if user selected the two folders
if (folder_orig_selected) and (folder_dest_selected):
    #check selected folder
    exist = files_function_library.check_folder(user_dest_path)
    text = ("Original Folder: " + str(user_dest_path) + " Exist: " + str(exist))
    log_record_functions.write_log_file (text)
    files = files_function_library.check_list_files(user_orig_path)
    ...
    for item in files:
        ...
        log_record_functions.write_log_file (file_orig_path)
        year, month = files_function_library.get_date_file (file_orig_path)
        ...
        log_record_functions.write_log_file (text)
        exist, created = files_function_library.check_create_folder(final_dest_path)
        ...
        if file_exist:
            ...
            error = files_function_library.copy_files_directory_error(file_orig_path , final_dest_path)
            text = ("Copy error: " + str(error))
            log_record_functions.write_log_file(text)
            

            copied_file_path = final_dest_path + "/" + item
            match = files_function_library.get_match_files(file_orig_path, copied_file_path)
            ...
            if ((match) and (not error) and pref_erase_origin):
                ...
                log_record_functions.write_log_file(text)
                files_function_library.erase_file(file_orig_path)
